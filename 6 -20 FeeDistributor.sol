...
contract ERC20DividendToken is ERC20, Ownable {
    ...
    constructor(string memory name_, string memory symbol_, address treasuryAddress_, address buyBackAddress_) ERC20(name_, symbol_, 9) {
        holderFee = 2;
        treasureFee = 7;
        burnFee = 4;
        buyBackFee = 3;
        ...
        sendTokensAtAmount = 100000000 * (10**9);
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ERC20DividendToken: Account is already the value of 'excluded'");
    ...
    function _transfer(address from,address to,uint256 amount) internal override {
        ...
        if (contractTokenBalance >= sendTokensAtAmount) {
          uint256 buyBackTokens = amount.mul(buyBackFee).div(totalFees);
          super._transfer(from, address(buyBackAddress), buyBackTokens);
          SendDividendsToBuyBackAddress(buyBackTokens);

          uint256 treasureTokens = amount.mul(treasureFee).div(totalFees);
          super._transfer(from, address(treasuryAddress), treasureTokens);
          SendDividendsToTreasury(treasureTokens);

          uint256 burnTokens = amount.mul(burnFee).div(totalFees);
...
contract XRXToken is ERC20DividendToken, VaultOwned {

    using SafeMath for uint256;

    address private _treasuryAddress = 0x0D87cc15fAcA9e2bC4BDe6c7aDA722918F4cb1A9;
    address private _buybackAddress = 0x3dBD199B1AA6DDb2aFCC68b720F3dB1B613E287C;

    constructor() ERC20DividendToken("XRX Token", "XRX", _treasuryAddress, _buybackAddress) {
    }
